 ...
 */
function catch_shop_register_option( $wp_customize, $option ) {
 ...
 */
function catch_shop_sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, 'catch_shop_' ) && 'catch_shop_important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
add_action( 'customize_register', 'catch_shop_sort_sections_list', 99 );
 ...
 * @since 1.0
 */
function catch_shop_section_visibility_options() {
	$options = array(
	...
	return apply_filters( 'catch_shop_section_visibility_options', $options );
}
 ...
 * @since 1.0
 */
function catch_shop_get_pagination_types() {
	$pagination_types = array(
		'default' => esc_html__( 'Default(Older Posts/Newer Posts)', 'catch-shop' ),
		'numeric' => esc_html__( 'Numeric', 'catch-shop' ),
	);

	return apply_filters( 'catch_shop_get_pagination_types', $pagination_types );
}
 ...
 * @return post_array
 */
function catch_shop_generate_post_array( $post_type = 'post' ) {
	$output = array();
