...
if( isset( $ftp_list ) && is_array( $ftp_list ) & !empty( $ftp_list )) {
	...
	<thead>
		<tr>
			<th><?php esc_html_e( "Profile name",'wp-migration-duplicator' ); ?></th>
			<th><?php esc_html_e( "Server Host/IP",'wp-migration-duplicator' ); ?></th>
	...
	foreach( $ftp_list as $key =>$ftp_item ) {
		...
		<tr>
			...
			<td>
				...
				<a class="wt_mgdp_ftp_edit wt_mgdp_action_btn" data-id="<?php echo esc_attr($ftp_item['id']); ?>"><?php esc_html_e('Edit','wp-migration-duplicator');?></a> | <a class="wt_mgdp_ftp_delete wt_mgdp_action_btn" data-id="<?php echo esc_attr($ftp_item['id']); ?>"><?php esc_html_e('Delete','wp-migration-duplicator');?></a>
			</td>
		</tr>
		<?php	
	}
	...
	?>
	<h4 style="margin-bottom:55px;"><?php esc_html_e("No FTP profiles found.",'wp-migration-duplicator'); ?></h4>
	<?php
}